...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-2">Record and manage teacher payments for {{ DateTime::createFromFormat('!m', $month)->format('F') }} {{ $year }}</p>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Teacher Payments Summary</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Earnings and payment status for each teacher</p>
        </div>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-lg font-bold text-green-600 dark:text-green-400">
                                        MAD {{ number_format($item['total_earnings'], 2) }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($item['is_paid'])
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-300">
                                            ...
                                            Paid (MAD {{ number_format($item['paid_amount'], 2) }})
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        @if($item['is_paid'])
                                            <a href="{{ route('teacher-payments.edit', TeacherPayment::where('teacher_id', $item['teacher']->id)->where('payment_month', $month)->where('payment_year', $year)->first()) }}"
                                        ...
                                        @else
                                            <a href="{{ route('teacher-payments.create', ['teacher_id' => $item['teacher']->id, 'month' => $month, 'year' => $year]) }}"
                                        ...
                                        @endif
                                        <a href="{{ route('reports.teacher-earnings', ['month' => $month, 'year' => $year]) }}?teacher_id={{ $item['teacher']->id }}"
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 dark:text-gray-400">No teachers are assigned to classes for the selected period.</p>
            </div>
        @endif
    </div>
</div>
