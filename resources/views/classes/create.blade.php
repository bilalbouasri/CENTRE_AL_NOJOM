            ...
            <div class="flex-1">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mt-2 text-lg">Create a new class for your educational center</p>
                    </div>
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('classes.store') }}">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        @foreach($commonGrades as $grade)
                            <div class="flex items-center p-3 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors duration-200">
                                <input type="checkbox" name="grade_levels[]" value="{{ $grade }}" id="grade_{{ $grade }}"
                                       ...
                                       {{ in_array($grade, old('grade_levels', [])) ? 'checked' : '' }}>
                ...
                <div class="lg:col-span-2">
                    ...
                    <p class="text-lg text-gray-600 dark:text-gray-400 mb-4">Add the weekly schedule for this class (2-3 sessions per week):</p>
                    
                    <div id="schedules-container" class="space-y-6">
                        <!-- Schedule template will be added here by JavaScript -->
                    </div>
                    
                    <button type="button" id="add-schedule"
                            class="mt-4 inline-flex items-center px-6 py-3 border-2 border-gray-300 dark:border-gray-600 rounded-xl shadow-lg text-base font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-800 transition-all duration-300">
                        <svg class="w-5 h-5 ltr:mr-3 rtl:ml-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addSchedule(data = {}) {
        ...
        const scheduleHtml = `
            <div class="schedule-item border-2 border-gray-200 dark:border-gray-600 rounded-xl p-6 bg-gray-50 dark:bg-gray-700 transition-all duration-300">
                <div class="flex justify-between items-center mb-4">
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white">Session ${index + 1}</h4>
                    <button type="button" class="remove-schedule text-red-600 hover:text-red-800 dark:hover:text-red-400 transition-colors duration-200 p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">Day</label>
                        <select name="schedules[${index}][day_of_week]" required
                                class="block w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl text-base focus:outline-none focus:ring-4 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-300 bg-white dark:bg-gray-600 text-gray-900 dark:text-white">
                    ...
                    <div>
                        <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">Start Time</label>
                        <input type="time" name="schedules[${index}][start_time]" required
                               value="${data.start_time || ''}"
                               class="block w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl text-base focus:outline-none focus:ring-4 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-300 bg-white dark:bg-gray-600 text-gray-900 dark:text-white">
                    </div>
                    <div>
                        <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">End Time</label>
                        <input type="time" name="schedules[${index}][end_time]" required
                               value="${data.end_time || ''}"
                               class="block w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl text-base focus:outline-none focus:ring-4 focus:ring-primary-500/20 focus:border-primary-500 transition-all duration-300 bg-white dark:bg-gray-600 text-gray-900 dark:text-white">
                    </div>
                    <div>
                        <label class="block text-base font-medium text-gray-700 dark:text-gray-300 mb-2">Duration (hours)</label>
